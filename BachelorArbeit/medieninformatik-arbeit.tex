...
The task started out as a interface for querying and visualising route calculations with algorithms implemented on the back end. Over time it evolved to a full blown mapping application with route calculations, point of interests, and many more.
\bigskip\\
...
To conclude this thesis a summary and recapitulation will be given in section \ref{sec:recap}. This section also includes interesting future work which is identified throughout the earlier sections.
\bigskip\\
...
average roundtrip to server: 34.806 ms
...
The next column displays the time, until the application starts to render the first elements. With 1.491 seconds the MARiO front end is in second place only beaten by HERE, which starts after a little less than a second. Noticeable is the very short time span of 0.222 seconds between first byte and start render for the HERE application. Optimising the render process of the MARiO front end would certainly reduce the time between first byte and start render significantly. 

The last column of table \ref{tab:basic} is the amount of Document Object Model (DOM) elements the website composed of, once it is fully loaded and rendered. The low number of 216 DOM elements are mostly the consequence of file and structure optimisation and has also impact on loading and rendering times.
\begin{table}[tbp]
\centering
\begin{tabular}{c|c|c|c|}
\cline{2-4}
...
paragraph 3
\bigskip\\
All performance tests were conducted with WebPagetest\footnote{www.webpagetest.org}, a testing framework primarily developed and run by Google. It allows the user to test performance with real life conditions in various locations around the globe. The tables above are the result of 15 tests performed in one hour intervals on August 16, 2016.
