...
As JavaScript is an untyped language, which means that variables don't have to be declared as one particular type, the developer receives almost no errors while compiling the code and therefore often stand before the problem of finding errors himself. The developers at Google "built many features into Angular which make testing"\footnote{https://docs.angularjs.org/guide/unit-testing} very easy. Two basic concepts of testing, unit tests and end-to-end tests, will be explained in the following two paragraphs.
