...
This subsection will contain all implemented features, describe them from a usability and technical standpoint and uncover shortcomings which will be an essential part of chapter six. The original features this thesis was constructed for, namely the calculation of routes using various algorithms and the calculation of intermodal routes will be explained in ~\ref{subsubsec:RouteCalculations} and ~\ref{subsubsec:Intermodal}. The remaining features are the points of interest search (see ~\ref{subsubsec:POI}), geocoding addresses (see ~\ref{subsubsec:Geocoding}) and the animated display of simulations of cars and thunderstorms which are the result of another students bachelors thesis (see ~\ref{subsubsec:Simulation}) (zitieren).
...
\label{subsubsec:RouteCalculations}
...
The second case comprises all algorithms from the skyline family. As delineated in 3.1 the skyline algorithms are able to optimise route calculations to more than one cost. That's why the user is able to select more than one or even all of the available costs in the direct route dropdown. The server returns a collection of routes in geojson format with the associated costs attached to the features. Adding the costs to the information panel of the front end instead of the addresses of the origin and destination allows the user to compare different routes in regard to their efficiency. The above mentioned route highlighting helps to differentiate between the displayed routes (see figure ~\ref{fig:HighlightingSkyline}).
...
The second main feature is the calculation of intermodal routes using public transportation such as trains and busses. This service is experimental and currently only available for the Berlin metro area.
